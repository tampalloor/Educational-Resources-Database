...
/**
 * BLMBD is a database which you can import, export, and display different resources for the BLM movement. 
 * In addition, you can sort by any order you like (Movie, OnlineRec, or Book)
 * You can sort and display by record type, identifier (name / title), author, director, description, year, and websiteURL. 
 * 
 * 
 * ------(For the code to work you would have to change the path of the text file to your computer on line 359)------
 *
 * @author David Nixon and Thomas Ampalloor :)
 * @version Summer 2020
 */
public class blmdb
{
	//uses RecordList class
    ...
    public static void main(String []args){
         runAllTests(); //Run all tests method, uncomment code from bottom if you want to test the full with menus
...
//        theDatabase.mainMenu();
    ...
    public void sortRecordsMenu(){
        ...
        String mainMessagePrompt = 
            ...
            "8. Go back to previous menu\n" +
            "(* Note: not all "
            + "Records have these attributes, so sorting is somewhat unpredictable.\n" +
            "   Generally records lacking the sorting attribute will be put at the bottom of the list.)\n" +
            "(Also note that you can sort by multiple criteria, simply by doing it in reverse order. For example,\n" +
            " if you want to sort by year, but have same-year items then sorted by name, just first sort by name, then year.)\n";
        ...
        while (theUserHasntYetChosenToQuit){
            ...
            switch(selection){
                ...
                case 2: // import from BLM_resources.txt 
                    this.importRecordsFromFile("BLM resources.txt");
    ...
    public static void runAllTests(){
        ...
        System.out.println();
        System.out.println("------------------TESTING IF RECORD LIST IS EMPTY------------------");
        if(testDatabase.recordList.isEmpty()) {
        	System.out.println("TEST PASSED, recordList is empty");
        }
        else {
        	System.out.println("TEST FAILED, record should be true because it is empty");
        }
        
        System.out.println();
        System.out.println("Importing all records from BLM Text");
        testDatabase.importRecordsFromFile("BLM resources.txt"); 
        System.out.println();
        System.out.println("------------------Printing individual Record Types------------------");
        System.out.println();
        System.out.println("------------------DISPLAY BY ONLINE------------------");
        testDatabase.recordList.displayRecordsOfRecordType("Online");
        System.out.println();
        System.out.println("------------------DISPLAY BY Movie------------------");
        testDatabase.recordList.displayRecordsOfRecordType("Movie"); 
        System.out.println();
        System.out.println("------------------DISPLAY BY Book------------------");
        ...
        System.out.println();
        System.out.println("----------------Sorting and printing by recordtype(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("recordType");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by identifier(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("identifier");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by description(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("description");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by director(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("director");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by author(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("author");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by website URL(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("websiteURL");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println();
        System.out.println("----------------Sorting and printing by year(ASCENDING ORDER)----------------");
        testDatabase.recordList.sortBy("year");
        testDatabase.recordList.displayAllRecords();
        System.out.println("------------------TESTING IF RECORD LIST IS EMPTY------------------");
        if(!testDatabase.recordList.isEmpty()) {
        	System.out.println("TEST PASSED, recordList is full");
        }
        else {
        	System.out.println("TEST FAILED, isEmpty() should be false");
        }
        System.out.println("------------------TESTING ADD RECORDS MENU------------------");
        testDatabase.importOneRecordFromConsole();
        System.out.println();
        System.out.println("------------------PRINTING ALL RECORDS WITH THE ADDED RECORD------------------");
        testDatabase.recordList.displayAllRecords(); 
        System.out.println("END RUN ALL TESTS METHOD (Overwrite is tested in the sort methods. Have a good day! :)");
